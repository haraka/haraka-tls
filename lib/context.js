...
class ContextStore {
  ...
  /**
   * Convenience: invalidate() + build(). Use this after certs on disk have
   * changed to drop every old SecureContext and create fresh ones.
   *
   * In-flight TLS connections retain their original SecureContext until they
   * close — only new handshakes pick up the new contexts.
   *
   * @param {object}                         base_opts
   * @param {Map<string, {key, cert}>}       certs
   */
  rebuild(base_opts, certs) {
    this.invalidate()
    this.build(base_opts, certs)
  }
}
