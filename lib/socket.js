...
class PluggableStream extends EventEmitter {
  ...
  _attach(socket) {
    ...
    socket.on('error', (err) => {
      this.writable = socket.writable
      err.source = 'tls'
      // Guard against late errors after listeners are removed — those would
      // otherwise become unhandled emissions and crash the process.
  ...
  const server = net.createServer((rawSocket) => {
    ...
    socket.upgrade = (upgrade_cb) => {
      ...
      cleartext.on('secure', () => {
        ...
        socket.emit('secure')
        upgrade_cb?.(cleartext.authorized, cleartext.authorizationError, cleartext.getPeerCertificate(), cipher)
      })
 ...
 * @param  {Function} [conn_opts.apply_mutual_tls] - optional (host, tls_opts) => tls_opts
 *                                                   called inside upgrade() so mutual-TLS
 *                                                   client certs are mixed in transparently
 * @returns {PluggableStream}
 */
function connect(conn_opts = {}) {
  const rawSocket = net.connect(conn_opts)
  const socket = new PluggableStream(rawSocket)
  const apply_mutual_tls = conn_opts.apply_mutual_tls

  socket.upgrade = (tls_opts = {}, upgrade_cb) => {
    log.debug('tls/socket: upgrading outbound connection to TLS')

    if (typeof apply_mutual_tls === 'function') {
      tls_opts = apply_mutual_tls(conn_opts.host, tls_opts)
    }
    ...
    cleartext.on('error', (err) => {
      // Log all TLS errors — err.reason is only set for a narrow subset of
      // failures, so gating logging on it hid most operational signal.
      log.error(`tls/socket: client TLS error: ${err.message || err}`)
      socket.emit('error', err)
    })
